...
def ecoSol(rou, nv, n, ncus, ncha, cDem, tMax, vCap, Cbat, arcDist, graDist, vUbDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, vL, EDist, objBest):
    ...
    m.addConstrs(Xs[ele] >= 0.5 for ele in rou if ele[0] not in range(ncus+1,n) and ele[1] not in range(ncus+1,n))
...
for filNcus in filNcuLs:
    ...
    cDem, tMax, vCap, Cbat, arcDist, graDist, vMaxDist, vMinDist, EMaxDist, EMinDist, twDist, tSer , arc, tMinDist, L, vL, EDis = dataRead(ncus,ncha,file,5/3.6) # vDis = 5 km/h
    ...
    sucess, eco_CalTime, Obj, Objgap, rouRes, vRou = ecoSol(rouIni, nv, n, ncus, ncha, cDem, tMax, vCap, Cbat, arcDist, graDist, vMaxDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, vL, EDis, 10**20)
    ...
    for ite in range(1,nIter): # Second-- Solutions
        ...
        if rouLi[-1] != rouLi[-2]: # Route !=, then we calculate the speed
            sucess, eco_CalTime, Obj, Objgap, rouRes, vRou = ecoSol(rouHeu, nv, n, ncus, ncha, cDem, tMax, vCap, Cbat, arcDist, graDist, vMaxDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc,  vL, EDis, obj_best)
