...
def sepCut(model,where):
    ...
    if where == GRB.Callback.MIPNODE:        
        ...
        if status == GRB.OPTIMAL:
            ...
            if numCnt == 0 and model._cbCuts <= 1000:
...
def ecoSol(rou, ncus, nv, n, cDem, tMax, vCap, Cbat, arcDist, graDist, vUbDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, objBest):
    ...
    m.addConstrs(X[ele] >= 0.5 for ele in rou)
    ...
    m.addConstrs(us[i,j] <= vUbDist[i,j]**2 for (i,j) in arc)
...
for file in files:
    ...
    ncus, nv, cDem, tMax, vCap, Cbat, arcDist, graDist, vMaxDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, tMinDist = dataRead(file)
    ...
    sucess, eco_CalTime, Obj, Objgap, rouRes, vRou = ecoSol(rouIni, ncus, nv, n, cDem, tMax, vCap, Cbat, arcDist, graDist, vMaxDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, 10**20)
    ...
    for ite in range(1,nIter): # Second-- Solutions
        ...
        if rouLi[-1] != rouLi[-2]: # Route !=, then we calculate the speed
            sucess, eco_CalTime, Obj, Objgap, rouRes, vRou = ecoSol(rouHeu, ncus, nv, n, cDem, tMax, vCap, Cbat, arcDist, graDist, vMaxDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, obj_best)         
