...
for file in files:
    ...
    ncus, nv, cDem, tMax, vCap, Cbat, arcDist, graDist, vUbDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, tMinDist = dataRead(file)
    ...
    try:
        startTime = time.time()
        m.update()
        m.read("ini_result/" + "nc_io-" + file.split('.')[0] + "_ini.sol")
