...
# =============================================================================
def rSol(n, nv, cDem, vCap, twDis, tSer, tDist, EDist, arc, cDist, ce, cb, cf, Cbat, localSearchTime): # number of customers, number of vehicles, demand, vehicle capacity, time windows, service time, travel time
    ...
    tMaxVal = max(tDist[key] for key in tDist.keys())
    EDist[0,n] = 0
    tDist[0,n] = 0
...
# =============================================================================
def rSol_ini(n, nv, cDem, vCap, twDis, tSer, tDist, EDist, arc, iniRou, cDist, ce, cb, cf, Cbat, localSearchTime): # number of customers, number of vehicles, demand, vehicle capacity, time windows, service time, travel time, initial route
...
# =============================================================================
# Callback, subtour elimination, subgradient cut
# =============================================================================
def sepCut(model,where):
               
    if where == GRB.Callback.MIPSOL: # check when an incumb is obtained
...
def ecoSol(rou, ncus, ncha, nv, n, cDem, tMax, vCap, Cbat, arcDist, graDist, vUbDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, objBest):
    ...
    m.addConstrs(X[ele] >= 0.5 for ele in [ele for ele in rou if ele in arc] if ele[0] not in range(ncus+1,n) and ele[1] not in range(ncus+1,n)) # fixed routes
    ...
    m.setParam("TimeLimit", 500) # Limit the solution time; Seconds
    ...
    if m.status in [2,9,10,11,13]:
        ...
        if Obj < objBest:
            m.write('ini_result/' + program.split('.')[0] + '-' + file.split('.')[0] + "_ini.sol")
...
for file in files: 
    ...
    ncus, ncha, nv, cDem, tMax, vCap, Cbat, arcDist, graDist, vMaxDist, vMinDist, EMaxDist, EMinDist, twDist, tSer, arc, tMinDist, arcRec = dataRead(file, numRep)
    ...
    # for ele in arcRec: # For the initial solution, we assign the arcs between recharging stations with a very big value, thus they are unlikely to be chosen
    #     tsDist[ele] = max(arcDist[ele]/vMinDist[ele] for ele in arc)
    #     esDist[ele] = max(EMaxDist.values())
        
    route_h, path_h = rSol(n, nv, cDem, vCap, twDis, tSer, tsDist, esDist, esDist.keys(), cDist, ce, cb, cf, Cbat, localSearchTime)
    rouIni = []
    ...
    for ite in range(1,nIter): # Second-- Solutions
        ...
        route_ht, path_ht = rSol_ini(n, nv, cDem, vCap, twDis, tSer, tsDist, esDist, arc, paths_best, cDist, ce, cb, cf, Cbat, localSearchTime)
        
        if path_ht != []: # Sometimes rSol_ini cannot give a feasible solution, then the path and route will be same as the ones in the last iteration
            route_h, path_h = route_ht, path_ht
        ...
        if rouLi[-1] != rouLi[-2] and path_ht != []: # Route !=, then we calculate the speed; if path_ht == [], we use the value in the last iteration
    ...
    ResDf.to_csv('ini_result/' + program.split('.')[0] + '_r' + str(numRep) + '.csv',index=True,sep=',',encoding='utf_8_sig')
