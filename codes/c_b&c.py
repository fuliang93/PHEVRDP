...
for filNcus in filNcuLs:
    ...
    cDem, tMax, vCap, Cbat, arcDist, graDist, vUbDist, vMinDist, EMaxDist, EMinDist, twDist, tSer , arc, tMinDist = dataRead(ncus,ncha,file)
    ...
    try:
        startTime = time.time()
        m.update()
        m.read("ini_result/" + "c_io-" + filNcus[0].split('.')[0] + '_' + str(filNcus[1]) + "_ini_solution.sol")
